...
`forward_fill_settlement()` — 交易所不定时发布结算参数表（SettlementParameters）时，未发布日的 `settle` / `maxup` / `maxdown` 字段在 DB 中为 NULL。

按合约分组、按日期排序，用最近一次非 NULL 值前向填充。

| 适用场景 | 说明 |
|----------|------|
| CFFEX SettlementParameters | 不定时发布，常见于假日前后 |
| DCE/INE 部分日期结算参数缺失 | 同上 |

## 与 `t_futures_info` 的预期差异及准许误差
...
通过 crontab 每日 16:30 执行完整流水线：

```
30 16 * * 1-5 /mnt/e/projects/data-sources/scripts/daily_report.sh
```
...
| `scripts/daily_report.sh` | crontab 定时流水线脚本 |
...
Writer 扫描原始目录时未找到该日对应文件，则该日该类型字段在 DB 记录中为 **NULL**（而非条目不存在）。Modifier 规则 12 `forward_fill_settlement()` 会按合约按日期排序，用上一次非 NULL 值前向填充。

**结果：** DB 中 CFFEX 结算/交易参数的 settle / maxup / maxdown 在未发布日从最后一次有效值继承，对齐旧表（Wind）行为。
