# data-sources — 期货数据管道

---

## 最近更新 (2026-05-29)

### 架构
- **DB 配置全部环境变量化**：无硬编码 IP/密码，`pipeline.sh` 是唯一配置入口
- **双库支持**：主库 (`DB_*`) + 比对库 (`REF_DB_*`)，通过 `config_override` 切换
- **数据/日志路径环境自适应**：`DATA_DIR`、`LOG_DIR` 根据 `DEV_MODE` 自动切换

### 数据格式
- **margin 统一为百分数**（Wind 口径）：12% → `12.0`，转换在 modifier 链末尾完成
- **date 归一化**：parser 输出 `YYYYMMDD`，modifier 统一为 `YYYY-MM-DD`
- **写入前先删**：`delete_rows()` 清空当日旧数据，避免 `ON DUPLICATE KEY UPDATE` 残留

### 依赖
- **移除 tushare**：`trade_dates.txt` 静态维护，每年手动追加次年交易日
- **新增 mxz-utils / data-sources wheel**：`pip install --user` 安装，生产无需 `PYTHONPATH`

### 部署
- **离线包打包**：`tar.gz` 包含源码 + data + deploy/offline/，不含 libs/config/docs
- **rutask 配置**：`/home/data_ops/rustask/rustask.local.json5`，task 名 `data_sources_pipeline`
- **数据目录**：`/data2_backup/data_sources_data/`，日志目录 `/home/data_ops/log/`
- **Playwright 浏览器**：`/home/data_ops/playwright-browsers/`

### 代码质量
- **清除死代码**：`compare_fields()`、`write_all()`、`_td()`、6 个重复 fetcher 方法
- **pylint 零容忍**：W0611/W0612/W0613/E0102/E0602 全部清零
- **生产统一**：不再依赖 pipx，全部使用 `python3 -m`

## 概述

从 CFE（中金所）、CZC（郑商所）、DCE（大商所）、GFE（广期所）、INE（能源中心）、SHF（上期所）六家期货交易所官网，定时下载结算参数、日行情、交易参数等原始数据，解析后写入 MySQL 数据库 `t_futures_info_exchange`。

目标：替代原 Wind 数据源填充的 `t_futures_info` 表，实现自主可控、全量覆盖的期货数据体系。

## 架构

```
Fetcher (下载) → Parser (解析) → Modifier (修正) → Writer (入库) → Reporter (报告/飞书)
                                                                      ↑
                                                              Verifier (对比验证)
```

## 数据流

### 下载

每天通过各交易所 API/网页 下载以下文件：

**七家交易所**（SHFE、INE、DCE、CZCE、CFFEX、GFEX）各三类核心文件：

| 文件类型 | 内容 |
|---------|------|
| `{date}.{exchange}.DailyMarketData.{ext}` | 日行情（OHLCV/oi/settle） |
| `{date}.{exchange}.SettlementParameters.{ext}` | 结算参数（settle/margin） |
| `{date}.{exchange}.TradingParameters.{ext}` | 交易参数（maxup/maxdown/margin/minoq/maxoq） |

**SHFE / INE** 额外下载产品配置 HTML：
| 文件类型 | 内容 |
|---------|------|
| `{date}.{exchange}.ProductConfig.html` | 产品规格配置页（`/products/` → Playwright 采集） |

**CSI（中证指数）** 下载指数日行情：
| 文件类型 | 内容 |
|---------|------|
| `{date}.CSI.MarketData.json` | 中证指数日行情（`/csindex-home/index-list/query-index-item`） |

文件保存至 `./data/raw/`，元数据（文件大小、变化率）记录在 `.metadata.jsonl`。

### 解析 → 修正 → 入库

```
parse_file() → merge_by_code_date() → _apply_modifiers() → upsert_records()
```

修正规则集中在 `modifier.py` 中按顺序执行（详见下方"Modifier 规则"）。

## Modifier 规则（人为设定的数据调整）

所有规则集中在 `modifier.py`，通过 `writer.py` 的 `_apply_modifiers()` 按序调用。

### 1. 节假日风控参数修正（核心规则）

> **背景**：交易所（DCE/CZC/SHFE/INE/GFE）在节假日前会调整涨跌停比例和保证金率（风控参数）。API 在调整日返回的是假日标准值，但 `t_futures_info`（原 Wind 表）中使用的是调整前的标准值。

**规则：date T 使用的涨跌停价和保证金率 = date T-1（前一交易日）API 的原始返回值。**

即：不重新计算，直接继承前一交易日的官方值。这确保新表与原表口径一致。

**适用交易所和对象：**

| 交易所 | 受影响字段 | 函数 |
|:-----:|:---------|------|
| DCE | `maxup`, `maxdown`, `long_margin`, `short_margin` | `fix_dce_limit_prices()` |
| CZC | `long_margin`, `short_margin` | `fix_all_margin()` → `_fix_margin_inherit()` |
| INE | `long_margin`, `short_margin` | 同上 |
| SHF | `long_margin`, `short_margin` | 同上 |
| GFE | `long_margin`, `short_margin` | `fix_gfe_margin()` |

### 2. 涨跌停取整精度修正

**背景**：`price × (1 + rate)` 的浮点运算会产生 `5940 × 1.10 = 6534.000000000001` 这类噪声，导致 `ceil(6534.000000000001) = 6535`（应为 6534）。

**方案**：使用 `safe_ceil(value, tick)` / `safe_floor(value, tick)` 替代原生 `math.ceil()` / `math.floor()`。函数内部先 `round(value / tick, 6)` 消除 FP 噪声再取整。

- `tick` 参数必传（最小变动价位）

### 3. TAS 合约结算价填充

`fill_tas_settle()` — TAS（结算价交易）合约的结算价等于同月份常规期货合约的结算价。

### 4. EFP 合约过滤

`should_filter_contract()` — EFP（期转现）合约在 DB 中无对应记录，入库前过滤。

### 5. 无 close 值合约 close 回退 settle

`fill_zero_volume_close()` — 当合约的 `close` 为空但 `settle` 有值时，将 `close` 设为 `settle`。

常见于以下情况：
- **零交易合约**：日行情文件只包含有交易的合约，未列出的零交易合约无 `close`
- **低成交近交割合约**：如 PX2605、eb2605 等 5 月到期合约，即使有少量成交（< 500 手），
  交易所行情中也无有效 OHLC 数据，但 Wind 等数据源会用 `settle` 填充 `close`

此规则与 Wind 的处理方式保持一致。

### 6. CSI 指数数据过滤

Writer 中过滤 `.CSI` 后缀的指数数据（`t_futures_info` 为纯期货数据表，不包含指数）。

### 7. GFE 非历史 API 保护

GFE 交易参数和日行情 API 不支持按日期查询历史数据（始终返回最新快照）。Fetcher 中自动跳过非当日的 GFEX 数据请求，保护已有历史文件不被当前快照覆盖。GFE 结算参数 API 使用数组格式 `["20260429"]` 可正确获取历史数据。

### 8. CFFEX margin 从历史结算文件继承

`fill_cffex_margin_from_history()` — CFFEX 的结算参数（margin）仅来自 SettlementParameters 文件，
该文件不定时发布（非每日更新）。当今日没有新结算文件时，从最近的历史 `*CFFEX.SettlementParameters.csv`
中读取 margin 值，回填到今日 CFFEX 记录的 `long_margin` / `short_margin` 中。

### 9. 数据库 schema

数值字段使用 `DECIMAL` 替代 `FLOAT`，避免大数值（如 `amt` 上亿元）的精度丢失。

## 可允许异常（与原表一致，非数据问题）

以下异常由数据源自身的限制导致，新旧表表现一致，不计为差异。

### 量小合约 OHLC 缺失（DCE 为主）

DCE 的低流动性合约（冷门品种或临近到期），API 不返回 Open/High/Low。
即使有成交（amt ≠ 0），open/high/low 也为 NULL，仅提供 close 和 settle。

典型场景：
- **冷门品种**（纯苯 BZ、焦炭 J 等）：成交量极低，API 仅返回主力合约 OHLC
- **临近到期合约**：进入交割月后无场内竞价，OHLC=0，被 `zero_price_to_none` 转换为 NULL

启发式识别规则（Verifier._classify_abnormal_null）：
- open/high/low 全部为 NULL
- close 有值
- volume < 300

**示例：**

**2026-05-06（冷门合约）**

| 合约 | open | high | low | close | volume | amt |
|------|:----:|:----:|:----:|:-----:|:-----:|:----:|
| bz2612.DCE | NULL | NULL | NULL | 7,532 | 1 | 226,000 |
| bz2701.DCE | NULL | NULL | NULL | 7,412 | 1 | 223,000 |
| j2703.DCE | NULL | NULL | NULL | 1,961 | 1 | 195,300 |
| j2704.DCE | NULL | NULL | NULL | 1,982 | 1 | 199,300 |

**2026-05-11（临近到期合约）**

| 合约 | open | high | low | close | settle | volume | amt |
|------|:----:|:----:|:----:|:-----:|:-----:|:-----:|:----:|
| a2605.DCE | NULL | NULL | NULL | 4,778 | 4,778 | 137 | 6,545,900 |
| eb2605.DCE | NULL | NULL | NULL | 9,834 | 9,834 | 49 | 2,409,300 |
| eb2703.DCE | NULL | NULL | NULL | 8,403 | 8,403 | 3 | 129,000 |

新旧表均表现为 NULL，该异常长期存在且可预期。

### INE TAS 合约字段缺失

SC（原油期货）的 TAS（结算价交易）合约由 INE DailyMarketData 生成记录，
但 TradingParameters 和 SettlementParameters 中不包含 TAS 合约的风控参数。

新表中已对 TAS 合约做入库前过滤（`should_filter_contract()`），TAS 合约不入库。
作为参考，TAS 合约缺失的字段包括：

| 字段 | 说明 |
|------|------|
| `oi` | 持仓量（TAS 无持仓数据） |
| `maxup` / `maxdown` | 涨跌停价（TAS 无此设定） |
| `long_margin` / `short_margin` | 保证金率（TAS 无此设定） |
| `volume` / `amt` | 成交量和金额（TAS 在日行情中另有统计） |

以上缺失属于 API 数据源的自然限制，非 pipeline 处理问题。

### 期货合约覆盖率

| 交易所 | 原表合约数 | 新表合约数 | 差异说明 |
|:-----:|:---------:|:---------:|---------|
| CFE | 28 | 28 | ✅ 一致 |
| CZC | 241 | 241 | ✅ 一致 |
| DCE | 250 | 250~253 | +3 月均价合约（F 后缀），原表无 |
| GFE | 48 | 48 | ✅ 一致 |
| INE | 67 | 67 | ✅ 一致 |
| SHF | 238 | 238 | ✅ 一致 |

### 字段数据 — 准许误差类别

#### B. GFE 涨跌停价 — 已知 API 限制

| 差异 | 原因 | 状态 |
|:----:|------|:----:|
| 48 条全部不同 | GFE 交易参数 API 不支持按日期查询，无法获取历史涨跌停价 | 已知限制 |

#### C. 数据源修正差异

CZCE 官网在收盘后可能对日行情数据进行微调。Wind 抓取的是修正前版本，新表使用的是终版数据。

| 合约 | 字段 | 原表 | 新表 | 差异 |
|:----:|:----:|:---:|:---:|:----:|
| SA2605.CZC | volume/oi/amt | 27,323 / 47,995 / 638,630,000 | **27,731** / **47,587** / **648,138,000** | +408 / -408 / +9,508,000 |

以交易所官网终版数据为准。

#### D. 成交额（amt）精度差

SHFE/INE 原始 API 返回的 TURNOVER 值包含更多小数位（如 `89434.945`），网页展示时保留两位（`89,434.95`）。原表是 FLOAT 类型，存储时精度丢失。新表使用 DECIMAL 类型，精度完整。

偏差范围：0.0001%~0.06%，绝对金额差 10~1000 元。认定为准许误差。

#### E. CZCE 成交额万元换算精度差

CZCE 成交额在文件中以万元为单位（2 位小数），×10000 换算为元时产生微小舍入差。

偏差范围：< 0.05%（绝大部分 < 0.01%）。认定为准许误差。

#### F. 无 close 合约

合约 `close` 为空时（零交易或低成交近交割），新表回退为 `settle`，与原表一致。不计为差异。

#### G. minoq/maxoq 字段

新表从 CZCE 交易参数文件中完整解析最小/最大开仓量，覆盖全部 241 条 CZC 合约。原表仅覆盖 36 条。新表数据更完整，不计为差异。

### 最终验证结果（20260429）

| 交易所 | 合约数 | 字段零差异率 | 说明 |
|:-----:|:------:|:----------:|------|
| CFE | 28/28 | 100% | ✅ |
| DCE | 250/250 | 100% | ✅（含修正后） |
| INE | 67/67 | 100% | ✅ |
| SHF | 238/238 | 100% | ✅ |
| CZC | 241/241 | >99% | SA2605 终版修正（准许），amt 微差（准许） |
| GFE | 48/48 | 0%（涨跌停） | API 限制（准许） |

## 定时任务

通过 crontab 交易日 16:30 执行完整流水线（脚本自行判定交易日，非交易日跳过）：

```
30 16 * * 1-5 /mnt/e/projects/data-sources/scripts/pipeline.sh
```

流水线步骤：
1. `Fetcher` — 下载各交易所当日原始数据
2. `Writer` — 解析 → Modifier 修正 → 写入 `t_futures_info_exchange`
3. `Reporter` — 生成三合一日度验证报告 → 飞书 + 邮件

   Reporter 的发件人（`--sender`）和收件人列表（`--recipients`）通过 pipeline.sh 以
   `python -m data_sources.reporter` 方式传入。当发件人或收件人地址不全时跳过邮件发送。

附加判定：
- **交易日判定**：检查 `data/trade_dates.txt`，非交易日跳过
- **时间判定**：16:27 前不执行（确保数据源已发布）

日志：`~/logs/cron.log`

## 公告监控与 order_limits 系统

### 背景

交易所的 minoq（最小开仓手数）、maxoq（最大下单手数）等参数，部分可从 fetcher 原始数据文件中解析（CZCE FutureTradeParam.txt、DCE tradingParam API），部分仅在交易所通用规则/品种公告中规定（CFFEX、GFEX、INE、SHFE）。后者不受 API/数据文件覆盖，需通过监控交易所公告来追踪调整。

### 四层规则体系

规则按优先级从低到高分为四层：

```
通用规则（交易所级） → 品种规则（品种级） → 调整规则（公告级） → 总结（当前值）
   层级1               层级2             层级3            层级4
```

| 文件 | 层级 | 行数 | 内容 |
|------|:---:|:----:|------|
| `data/order_limits_general_rules.csv` | 1 | 7 | 各交易所"交易管理办法"/"交易细则"中的交易所级通用规则，含原文出处（evidence列） |
| `data/order_limits_product_rules.csv` | 2 | 27 | 各品种"业务细则"/"品种公告"中的品种级规定，列出偏离通用规则的例外品种 |
| `data/order_limits_adjustments.csv` | 3 | 17 | 按时间排序的公告变更，逐条记录调整前→调整后的指标值，含公告编号和发布日期 |
| `data/order_limits_summary.csv` | 4 | 20 | 通用+品种+调整三层合并后的当前值，**仅覆盖 fetcher 拿不到的交易所**（CFFEX/GFEX/INE/SHFE） |
| `data/raw/announcements/metadata.jsonl` | — | 5 | 公告元数据索引：公告编号、标题、URL、发布时间、涉及字段、爬取时间 |
| `data/raw/announcements/` | — | — | 公告原文 PDF/HTML |

### 设计原则

- **fetcher 有的不重复**：CZCE/DCE 合约的 minoq/maxoq 来自原始数据文件（FutureTradeParam.txt / tradingParam API），summary 中不列示
- **规则分层可追溯**：每层独立维护，summary 的 evidence 列追溯值来自哪几层规则
- **公告元数据独立**：`metadata.jsonl` 是所有公告的统一索引，`order_limits_adjustments.csv` 只记录对指标有实际影响的调整
- **品种例外显式列出**：品种规则中只列偏离通用规则的品种（如红枣 100/20、动力煤 50/10），其余品种默认沿用通用规则

### 相关脚本

| 脚本 | 职责 |
|------|------|
| `scripts/announcement_watcher.py` | 各交易所公告爬取 + LLM 分析变更 |
| `scripts/check_announcements.py` | 公告变更检测（pipeline Step 4） |
| `scripts/order_limits_updater.py` | 检测到 minoq/maxoq 变更后自动更新 CSV |
| `scripts/order_limits_report.py` | 按合约查询 minoq/maxoq 溯源报告 |

### 爬取注意事项

- **GFEX** 页面 HTML 中嵌有精确发布时间（如 `2025-12-24T21:43:00`）
- **DCE / CZCE** 页面只显示发布日期，不发布具体时刻
- CZCE 公告编号格式：`〔2026〕XX号`；DCE：`大商所发〔2026〕XX号`；GFEX：`广期所公告〔2026〕XX号`

---

## 浏览器配置（Playwright）

### 在 venv 中使用项目自带 Chromium

项目部署包（`deploy/offline/playwright/`）内含离线 Chromium 二进制，无需联网安装：

```bash
cd /mnt/e/projects/data-sources
source .venv/bin/activate

# 指向 deploy 目录的浏览器
export PLAYWRIGHT_BROWSERS_PATH="$PWD/deploy/offline/playwright"
export DISPLAY=:0              # 有头模式需要 X11

# 使用
python -c "
from playwright.sync_api import sync_playwright
with sync_playwright() as p:
    browser = p.chromium.launch(headless=False, args=['--no-sandbox'])
    print(browser.version)
    browser.close()
"
```

### 已知限制

- Ubuntu 26.04 不被 Playwright 官方支持（`playwright install chromium` 会报错 "does not support ubuntu26.04"）。使用 `deploy/offline/playwright/` 中的 chromium-1208 即可
- 部分交易所网站（DCE/CZCE）有 JS 反爬，headless 模式可能被拦截——有头模式（`headless=False`）通常能通过
- 如缺少系统库（libnspr4/libnss3/libasound2 等），可以从项目 depoly 目录或 apt 安装

---

## 关键文件

| 文件 | 职责 |
|------|------|
| `src/data_sources/fetcher.py` | 数据下载（各交易所 API） |
| `src/data_sources/parser.py` | 原始文件解析 |
| `src/data_sources/modifier.py` | **数据调整规则（本文档核心）** |
| `src/data_sources/writer.py` | 修正链编排 + DB 写入 |
| `src/data_sources/verifier.py` | 双表全面对比验证 |
| `src/data_sources/reporter.py` | 日度验证报告 + 飞书通知 + 邮件（`python -m data_sources.reporter` CLI） |
| `src/data_sources/db.py` | 数据库连接与 schema |
| `src/data_sources/configs.py` | 任务配置 + URL 常量（取代 constants.py + fetcher_task_configs/） |
| `src/data_sources/task.py` | Task / TaskConfig 数据模型（取代 models.py） |
| `scripts/pipeline.sh` | crontab 定时流水线脚本 |

---

## 交易日历工具（trade_date.py）

**文件位置：** `src/data_sources/trade_date.py`
**数据文件：** `data/trade_dates.txt`（纯文本 8797 行）

### 存储方案

纯文本，每行一个 YYYYMMDD 整数，升序排列。

```
19901219
19901220
...
20261231
```

### 方案取舍

| 方案 | 舍弃原因 |
|------|----------|
| JSON 字符串数组 | 生成 Python 对象前需 json.load 完整解析，shell 查日期需 jq |
| JSON 整型数组 | 同上，shell 不友好 |
| SQLite | 为 9k 行数据开一个 DB 太重，运维成本高 |
| Python .py 字面量 | Python 内最快，shell 无法直接读取 |
| ✅ 纯文本逐行 | shell 原生支持（grep/awk 零依赖），Python 同等高效 |

### 核心原因

**1. Shell 原生操作，零格式转换**

运营场景需要在命令行快速读取并赋值环境变量：

```bash
LAST_TRADE_DATE=$(awk -v d=20260506 '$1<=d{last=$1} END{print last}' data/trade_dates.txt)
```

日期 `20260506` 在 shell 中是裸整数，不需要引号，不需要 `jq`，不需要 `date -d` 转格式。纯文本文件一行一个整数，grep/awk 直接操作。

**2. Python 模块级缓存，一次读盘**

`load()` 首次调用读入文件后驻留内存（`list[str]` ≈ 80KB），后续所有查询走二分查找 O(log n)，不再读盘。

**3. 快速人工 hack**

临时调整只需一条 shell 命令修改 `trade_dates.txt`，重启进程即刻生效，不依赖任何数据源：

```bash
# 增加交易日
echo "20260203" >> data/trade_dates.txt && sort -o data/trade_dates.txt data/trade_dates.txt
# 去掉交易日
sed -i '/^20260131$/d' data/trade_dates.txt
```

### 数据源策略

| 层级 | 方式 | 频率 |
|------|------|------|
| 手动 | 直接编辑 trade_dates.txt | 按需 |

> **每年更新**：`trade_dates.txt` 由 tushare（已废弃）一次性生成后静态维护，
> 每年末需手动追加下一年的交易日数据。
> 上交所通常在每年 12 月发布次年交易日历，届时编辑 `data/trade_dates.txt` 追加即可。

### 接口

```python
def load(force: bool = False) -> list[str]                     # 读文件（模块缓存，force=True 强制刷盘）
def nearest(d) -> Optional[str]                                # 含自身，向前最近
def prev_trading_date(d) -> Optional[str]                      # 不含自身，真正上一天
def is_trading(d) -> bool                                      # 是否交易日
```

手工编辑 `trade_dates.txt` 后刷新缓存：

```python
from trade_date import load, nearest
load(force=True)                # 重新读盘
n = nearest(20260506)           # 用新数据查询
```

CLI：`python trade_date.py nearest|prev|is_trading <YYYYMMDD>|--update`


## CFFEX 结算参数表（不定时发布）

### 背景

CFFEX（中金所）有三种文件：

| 文件 | 发布频率 |
|------|----------|
| `DailyMarketData`（日行情） | **每日**发布，每个交易日都有 |
| `SettlementParameters`（结算参数表） | **不定时**，仅在结算参数变化时发布 |
| `TradingParameters`（交易参数表） | **不定时** |

### 无发布日的处理

Fetcher 每天仍会尝试下载。CFFEX 当天未发布时服务器返回 HTML 错误页，Fetcher 识别后标记 `no_data` 不报错：

```python
# fetcher.py _fetch_cffex_settlement
if "HTML error page" in reason:
    return {"success": True, "no_data": True}
```

Writer 扫描原始目录时未找到该日对应文件，则该日该类型字段在 DB 记录中为 **NULL**（而非条目不存在）。日行情文件（DailyMarketData）每日发布，其余字段正常写入。

继承逻辑：`modifier.py` 中的 `fill_cffex_margin_from_history()` 在 Writer 中自动调用，
当今日无结算文件时，从最近的非今日 `*CFFEX.SettlementParameters.csv` 中
读取 margin 值回填，确保 `long_margin` / `short_margin` 不因未发布日而缺失。

---

## 构建与部署

### 环境要求

| 组件 | 版本 |
|------|------|
| Python | >= 3.10 |
| pipx | 任意版本 |
| MySQL | 8.0+（目标库 `future_cn`） |

### 前置依赖安装

```bash
# 安装 pipx（如未安装）
python3 -m pip install --user pipx
python3 -m pipx ensurepath

# 安装 Playwright 浏览器（数据抓取需要）
pipx run playwright install chromium
```

### 构建与安装

项目根目录执行：

```bash
cd /mnt/e/projects/data-sources
pipx install .
```

安装后生成三个全局 CLI 命令：

| 命令 | 说明 |
|------|------|
| `fetcher` | 数据下载 |
| `writer` | 解析 → 修正 → 入库 |
| `reporter` | 日度验证报告 → 飞书 + 邮件 |

> **开发模式**：设置环境变量 `DEV_MODE=1` 后 `pipeline.sh` 自动使用本地源码（`python -m`），
> 无需每次修改后重新安装。

### 生产部署

#### 1. 推送代码

```bash
# GitHub 远端
cd /mnt/e/projects/data-sources
git push origin main

# 公司 GitLab 远端
git push gitlab main
```

#### 2. 生产服务器拉取与安装

```bash
cd /mnt/e/projects/data-sources
git pull origin main          # 或 git pull gitlab main
pipx install . --force        # 升级已安装的 app
```

#### 3. crontab 配置

```
30 16 * * 1-5 /bin/bash /mnt/e/projects/data-sources/scripts/pipeline.sh >> ${HOME}/logs/cron.log 2>&1
```

`pipeline.sh` 自动处理：
- 交易日判定（非交易日跳过）
- 时间门禁（16:27 前不执行，等待数据发布）
- pipx 命令检测与降级（未安装时自动退回 `python -m` 开发模式）

#### 4. 环境变量

| 变量 | 必填 | 说明 |
|------|:----:|------|
| `SMTP_PASSWORD` | ✅ | 企业微信邮箱 `mxz@wendao.fund` 的登录密码 |
| `FEISHU_ALERT_URL` | ❌ | 飞书告警机器人 webhook（可选） |
| `DEV_MODE` | ❌ | 设为 `1` 时走本地源码模式 |

SMTP 密码与收件人地址已在 `scripts/pipeline.sh` 中管理。

### 验证

```bash
# 验证三个 CLI 命令可用
fetcher --help
writer --help
reporter --help

# 开发模式测试指定日期
DEV_MODE=1 bash scripts/pipeline.sh 20260507

# 生产模式运行（先确认已 pipx install）
bash scripts/pipeline.sh
```

### 日志

| 文件 | 说明 |
|------|------|
| `${HOME}/logs/cron.log` | pipeline.sh stdout/stderr（crontab 流水线日志） |
| `./logs/fetcher.*.log` | Fetcher 模块的 AlertLogger 日志 |
| `./logs/writer.*.log` | Writer 模块的 AlertLogger 日志 |
| `./logs/reporter.*.log` | Reporter 模块的 AlertLogger 日志 |

日志文件按日期轮转（`HybridRotatingFileHandler`），单文件上限 100MB，保留 30 天。

## 已知问题

### DCE 到期合约缺少涨跌停价

每月最后交易日，交易所会在当日 TradingParameters 中摘除到期合约，导致 maxup/maxdown 为 NULL。DailyMarketData 仍有行情数据（close/volume/amt/oi）。

影响范围：到期月份的合约（如 5/26 为 2605 合约最后交易日），通常 5–10 个合约。这些合约在 `t_futures_info`（Wind 源）中有完整数据，两表对比会出现差异。

验证：前一天（5/25）的 TradingParameters 中含有到期合约（eg2605/bz2605/jd2605/lh2605/eb2605/lg2605/pg2605），确认是到期摘牌导致。

### Wind WSS `code_col=code` 与 `minoq,maxoq` 字段冲突

调用 `cpp_py.w.wss()` 时，如果 options 中包含 `code_col=code`，同时查询字段含 `minoq` 或 `maxoq`，会返回 `CWSSService: invalid indicators` 错误（ec=-40522006）。

**解决办法**：去掉 `code_col=code` 选项。去掉后 ec=0，数据按输入代码顺序返回，code 列不输出，需要通过行号映射回对应合约。

**已知能用**：`code_col=code` + `open,close,volume,...` 等标准字段无此问题。仅 `minoq/maxoq` 两个字段受影响。
