...
def _pw_download(url: str, local_path: Path):
    ...
    try:
        ...
        _llm_logger.info("附件下载成功: %s (%s bytes)", local_path.name, local_path.stat().st_size)
        return local_path
    finally:
        ctx.close()
        page.close()
...
async def _download(
    ...
    if local.exists():
        _llm_logger.info("附件下载成功: %s (%s bytes)", local.name, local.stat().st_size)
        return local
        ...
        try:
            async with _host_semaphore(url):
                async with session.get(
                    ...
                    tmp.write_bytes(body)
                    tmp.rename(local)
                _llm_logger.info("附件下载成功(aiohttp): %s (%s bytes)", local.name, local.stat().st_size)
                return local
        except Exception as e:
            last_err = e
            if attempt < 2:
    ...
    try:
        ...
        tmp.write_bytes(body)
        tmp.rename(local)
        _llm_logger.info("附件下载成功(requests兜底): %s (%s bytes)", local.name, local.stat().st_size)
        return local
    except Exception:
        pass
    raise last_err
